...
impl Default for IsingApp {
    fn default() -> Self {
        return Self {
            model: IsingModel::new(100, 2.269, Some(1.0), InitialCondition::Random),
            steps_per_frame: 10,
            is_running: false,
            texture: None,
...
impl eframe::App for IsingApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        if self.is_running {
            ...
            let current_energy = self.model.energy / (self.model.l * self.model.l) as f64;
            ...
            for i in (self.history_mag.len() - window)..self.history_mag.len() {
                sum_mag += self.history_mag[i][1];
                sum_mag_squared += self.history_mag[i][1].powf(2.0);
            }
            let n = self.model.l * self.model.l;
            let current_susceptibility = self.model.beta
                * (n as f64)
                * (sum_mag_squared / window as f64 - (sum_mag / window as f64).powf(2.0));
        ...
        egui::SidePanel::left("control_panel").show(ctx, |ui| {
            ...
            ui.label(format!(
                "Energy: {:.3}",
                self.model.energy / (self.model.l * self.model.l) as f64
            ));
